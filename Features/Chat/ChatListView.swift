                    ...
                    LazyVStack(spacing: 0) {
                        ForEach(filtered) { conv in
                            ConversationRow(conversation: conv) {
                                router.navigate(to: .chat(conversationId: conv.id))
                            }
...
struct ChatDetailView: View {
    ...
    private func sendMessage() {
        ...
        let msg = ChatMessage(id: UUID().uuidString, conversationId: "", senderId: "me", content: newMessage, type: .text, status: .sent, createdAt: Date())
...
extension ChatMessage {
    static let mocks: [ChatMessage] = [
        ChatMessage(id: "m1", conversationId: "1", senderId: "u1", content: "Olá! Vi que você se interessou pela minha sala odontológica.", type: .text, status: .delivered, createdAt: Date().addingTimeInterval(-3600)),
        ChatMessage(id: "m2", conversationId: "1", senderId: "me", content: "Sim! Queria saber se está disponível para o dia 15.", type: .text, status: .delivered, createdAt: Date().addingTimeInterval(-3500)),
        ChatMessage(id: "m3", conversationId: "1", senderId: "u1", content: "Está sim! Qual horário você precisa?", type: .text, status: .delivered, createdAt: Date().addingTimeInterval(-3400)),
        ChatMessage(id: "m4", conversationId: "1", senderId: "me", content: "Das 9h às 17h, o dia todo.", type: .text, status: .delivered, createdAt: Date().addingTimeInterval(-3300)),
        ChatMessage(id: "m5", conversationId: "1", senderId: "u1", content: "Perfeito! Confirmo a reserva para sexta.", type: .text, status: .sent, createdAt: Date().addingTimeInterval(-600)),
    ]
}
