...
private struct MenuItem: Identifiable {
    ...
    init(_ label: String, icon: String, subtitle: String? = nil, badge: Int = 0, destination: AppDestination? = nil, destructive: Bool = false) {
        self.label = label
        self.icon = icon
    ...
    private var sections: [MenuSection] {
        [
            MenuSection(title: "Descoberta", items: [
                MenuItem("Home",         icon: "house.fill",          destination: .home),
                MenuItem("Buscar",       icon: "magnifyingglass",     destination: .home),
                MenuItem("Filtros",      icon: "slider.horizontal.3", destination: nil),
            ]),
            MenuSection(title: "Minha Conta", items: [
                MenuItem("Perfil",               icon: "person.crop.circle",   destination: .profile(userId: "")),
                MenuItem("Meus Anúncios",        icon: "list.bullet.rectangle", destination: .home),
                MenuItem("Minhas Reservas",      icon: "bag.fill",              destination: nil),
                MenuItem("Favoritos",            icon: "heart.fill",            destination: nil),
            ]),
            MenuSection(title: "Comunicação", items: [
                MenuItem("Chat",           icon: "bubble.left.and.bubble.right.fill", badge: 2, destination: nil),
                MenuItem("Notificações",   icon: "bell.fill",  badge: 1, destination: .notifications),
            ]),
            MenuSection(title: "Crescimento", items: [
                MenuItem("Indique e Ganhe (MGM)", icon: "gift.fill", subtitle: "Ganhe recompensas por indicação", destination: .mgm),
            ]),
            MenuSection(title: "Suporte", items: [
                MenuItem("Ajuda e FAQ",    icon: "questionmark.circle.fill", destination: nil),
                MenuItem("Termos de uso",  icon: "doc.text.fill",            destination: nil),
                MenuItem("Sair da conta",  icon: "rectangle.portrait.and.arrow.right", destination: nil, destructive: true),
            ]),
        ]
    }
            ...
            HStack(spacing: CRSpacing.md) {
                ...
                if item.badge > 0 {
                    Text("\(item.badge)")
                        ...
                        .clipShape(Capsule())
                } else if !item.isDestructive && item.destination != nil {
